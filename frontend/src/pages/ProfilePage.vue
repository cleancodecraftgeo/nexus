...
<template>
  <section class="max-w-3xl mx-auto px-4 py-3 border rounded mt-2 relative">
    <div class="flex justify-between mb-6 ">
      <h1 class="md:text-2xl font-semibold ">{{t('profile.title')}}</h1>
    ...
    <div v-if="authStore.user" class="border rounded-xl p-6 space-y-3">
      <p>
        <span class="font-medium">{{t('profile.name')}}: </span>{{ authStore.user.name }}
      </p>
      <p>
        <span class="font-medium">{{t('profile.email')}}: </span>{{ authStore.user.email }}
      </p>
      <p>
        <span class="font-medium">{{t('profile.role')}}: </span>{{ authStore.user.role }}
      </p>
    </div>
    ...
    <form v-if="isEditing" @submit.prevent="updateProfile"
      ...
      <div>
        <label for="name" class="block text-gray-100 mb-1 ">{{t('profile.name')}}</label>
        <input id="name" v-model="form.name" type="text" class="border text-gray-100 rounded px-3 py-2 w-full" />
      </div>

      <div>
        <label for="email" class="block text-gray-100 mb-1">{{t('profile.email')}}</label>
